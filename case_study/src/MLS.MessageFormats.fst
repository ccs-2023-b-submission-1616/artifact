...
val leaf_node_lifetime: leaf_node_source -> Type0
let leaf_node_lifetime source =
  match source with
  | LNS_key_package -> lifetime
  | _ -> unit

%splice [ps_leaf_node_lifetime] (gen_parser_prefix (`leaf_node_lifetime))

val leaf_node_parent_hash: bytes:Type0 -> {|bytes_like bytes|} -> leaf_node_source -> Type0
let leaf_node_parent_hash bytes #bl source =
  match source with
  | LNS_commit -> mls_bytes bytes
  | _ -> unit

%splice [ps_leaf_node_parent_hash] (gen_parser_prefix (`leaf_node_parent_hash))
...
type leaf_node_data (bytes:Type0) {|bytes_like bytes|} (tkt:treekem_types bytes) = {
  ...
  lifetime: leaf_node_lifetime source;
  parent_hash: leaf_node_parent_hash bytes source;
  extensions: mls_list bytes ps_extension;
}
...
val leaf_node_tbs_group_id: bytes:Type0 -> {|bytes_like bytes|} -> leaf_node_source -> Type0
let leaf_node_tbs_group_id bytes #bl source =
  match source with
  | LNS_update
  | LNS_commit -> mls_bytes bytes
  | _ -> unit

%splice [ps_leaf_node_tbs_group_id] (gen_parser_prefix (`leaf_node_tbs_group_id))

val leaf_node_tbs_leaf_index: bytes:Type0 -> {|bytes_like bytes|} -> leaf_node_source -> Type0
let leaf_node_tbs_leaf_index bytes #bl source =
  match source with
  | LNS_update
  | LNS_commit -> nat_lbytes 4
  | _ -> unit

%splice [ps_leaf_node_tbs_leaf_index] (gen_parser_prefix (`leaf_node_tbs_leaf_index))

type leaf_node_tbs (bytes:Type0) {|bytes_like bytes|} (tkt:treekem_types bytes) = {
  data: leaf_node_data bytes tkt;
  group_id: leaf_node_tbs_group_id bytes data.source;
  leaf_index: leaf_node_tbs_leaf_index bytes data.source;
}
...
let framed_content_tbs_group_context (bytes:Type0) {|bytes_like bytes|} (s:sender bytes) =
  match s with
  | S_member _
  | S_new_member_commit -> group_context bytes
  | S_external _
  | S_new_member_proposal -> unit

%splice [ps_framed_content_tbs_group_context] (gen_parser_prefix (`framed_content_tbs_group_context))

type framed_content_tbs (bytes:Type0) {|bytes_like bytes|} = {
  version: protocol_version;
  wire_format: wire_format;
  content: framed_content bytes;
  group_context: framed_content_tbs_group_context bytes (content.sender);
}
...
val confirmation_tag: bytes:Type0 -> {|bytes_like bytes|} -> content_type -> Type0
let confirmation_tag bytes #bl content =
  match content with
  | CT_commit -> mac bytes
  | _ -> unit

%splice [ps_confirmation_tag] (gen_parser_prefix (`confirmation_tag))

type framed_content_auth_data (bytes:Type0) {|bl:bytes_like bytes|} (content_type:content_type) = {
  signature: mls_bytes bytes;
  confirmation_tag: confirmation_tag bytes #bl content_type;
}
...
instance parseable_serializeable_authenticated_content_tbm (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (authenticated_content_tbm bytes) = mk_parseable_serializeable ps_authenticated_content_tbm
...
let membership_tag (bytes:Type0) {|bytes_like bytes|} (s:sender bytes) =
  match s with
  | S_member _ -> mac bytes
  | _ -> unit

%splice [ps_membership_tag] (gen_parser_prefix (`membership_tag))

type public_message (bytes:Type0) {|bytes_like bytes|} = {
  content: framed_content bytes;
  auth: framed_content_auth_data bytes content.content.content_type;
  membership_tag: membership_tag bytes content.sender;
}
...
instance parseable_serializeable_private_message_content (bytes:Type0) {|bytes_like bytes|} (content_type:content_type): parseable_serializeable bytes (private_message_content bytes content_type) = mk_parseable_serializeable_from_whole (pse_private_message_content content_type)
...
instance parseable_serializeable_private_content_aad (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (private_content_aad bytes) = mk_parseable_serializeable ps_private_content_aad
...
instance parseable_serializeable_sender_data (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (sender_data bytes) = mk_parseable_serializeable ps_sender_data
...
instance parseable_serializeable_sender_data_aad (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (sender_data_aad bytes) = mk_parseable_serializeable ps_sender_data_aad
...
instance parseable_serializeable_confirmed_transcript_hash_input (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (confirmed_transcript_hash_input bytes) = mk_parseable_serializeable ps_confirmed_transcript_hash_input
...
instance parseable_serializeable_interim_transcript_hash_input (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (interim_transcript_hash_input bytes) = mk_parseable_serializeable ps_interim_transcript_hash_input
...
instance parseable_serializeable_group_info_tbs (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (group_info_tbs bytes) = mk_parseable_serializeable ps_group_info_tbs
...
instance parseable_serializeable_group_info (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (group_info bytes) = mk_parseable_serializeable ps_group_info
...
instance parseable_serializeable_group_secrets (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (group_secrets bytes) = mk_parseable_serializeable ps_group_secrets
...
instance parseable_serializeable_mls_message (bytes:Type0) {|bytes_like bytes|}: parseable_serializeable bytes (mls_message bytes) = mk_parseable_serializeable ps_mls_message
